...
<html lang="en">
 ...
 echo " <tbody>";
    ...
    if($resultado != NULL){
        foreach($resultado as $chave  => $pag){
          
            echo "<tr>";
            echo "<th scope='row'>{$pag->getIdPagamento()}</th>
                              <td>{$pag->getNome()}</td>
                              <td><a href='atualizarfarmacia.class.php?idFarmacia={$pag->getIdPagamento()}'><i class='material-icons'> update </i></a></td>
                              <td><a href='deletarfarmacia.php?idFarmacia={$pag->getIdPagamento()}'><i class='material-icons'> delete </i></a></td>
...
?>
  ...
  <a href="cadastropagamento.php" class="btn btn-primary btn-lg active" role="button" aria-pressed="true">Cadastrar Pagamento</a>  
    <!-- Optional JavaScript -->
